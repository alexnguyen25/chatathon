...
- Xcode project: `ios/LowCor.xcodeproj`; scheme: `LowCor`; Xcode 26+, iOS 26+.
- App entry: `ios/LowCor/App/LowCorApp.swift` → `Views/TodayView.swift`.
- Shared logic: `ios/LowCor/Core`. The root `Package.swift` compiles these same sources for `swift test`; tests belong in `Tests/LowCorCoreTests`.
- Integrations: `Health`, `Calendar`, and `Suggestions` inside `ios/LowCor`.
- Visual design reference: `docs/design/lowcor-core`; retain the research and design history.
